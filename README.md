...
- [**延时基准 v1**](docs/benchmarks/latency/benchv1.md) — 基础架构的延时（Baseline）测试。
- [**延时优化 v2**](docs/benchmarks/latency/benchv2.md) — `SPSC` 队列 + 线程绑核 (Pinned) 与消除 JSON/String 的开销测试。
- [**延时优化 v3**](docs/benchmarks/latency/benchv3.md) — 采用新的 HashMap 和基于 LOB (Limit Order Book) 的优化测试，订单薄操作延迟低至 100ns 级别。
- [**端到端延时 v4**](docs/benchmarks/latency/benchv4.md) — 最终版的端到端 (E2E) Ping-Pong 延迟测试。
...
#### `docs/benchmarks/latency` 专项延时优化

我们在后期的开发中重点针对核心链路的极低延时（Ultra-low Latency）进行了打磨，采用软硬结合的手段逐点击破：

1. **`latency/benchv1.md` / `latency/benchv2.md`**：记录了采用 `SPSC` (单生产者单消费者) 队列，结合**线程绑核（Pinning）** 策略的表现。测试中通过直接传入 `Order` 结构体绕过热路径上的 `nlohmann::json` 以及 `std::string` 深拷贝，消除了大量堆分配导致的系统调用和缓存抖动，大幅拉低了系统在单生产者、多 Bucket (消费者) 架构下的 E2E 延迟。
2. **`latency/benchv3.md`**：记录了采用自行实现的 `FastHashmap` 和基于 `PriceBitSet`、对象池构建的数组化 LOB (Limit Order Book) 的纯撮合专项测试。剥离风控和路由开销的微基准测试表明，引擎在撮合发生成交（Hit）时的平均延时达到了 103 ns（仅三百多 CPU cycles），未命中（Miss）时延低至 ~60 ns。
3. **`latency/benchv4.md`**：记录了最终版系统的端到端 (E2E) 延时表现。测试采用**限制提交速率（Paced）**而非简单循环的方法来逼近系统无排队情况下的物理极速（包含：SPSC push → worker pop → dispatch → matching → callback）。结果显示，在极速路径下处理一笔完整交易的延时 P50 在 806.0 ns，P90 约 1034.3 ns，证明系统已达成亚微秒级的全链路处理能力（Zero-Heap 开销）。
...
- 撮合引擎核心实现：
  - [PR #13：feat: 实现撮合引擎核心功能 (模块B)](https://github.com/JunkuiZhang/hairline-defense-force/pull/13)（[@汤语涵](https://github.com/Animnia)）
  - 撮合模块部分优化：
